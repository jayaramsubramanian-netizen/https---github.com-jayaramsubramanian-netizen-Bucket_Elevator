    ...
    <div style={{ padding: "10px 10px 0" }}>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 10 }}>
        <Tile
          ...
          label="Pass"
          color="var(--success)"
          dim="rgba(31,184,110,.08)"
        />
        <Tile
          count={warn}
          label="Warn"
          color="var(--warning)"
          dim="rgba(217,142,0,.08)"
        />
        <Tile
          count={fail}
          label="Fail"
          color="var(--danger)"
          dim="rgba(224,82,82,.08)"
        />
        <Tile
          count={info}
          label="Info"
          color="var(--text3)"
          dim="rgba(90,122,154,.08)"
        />
      </div>
      ...
      {pct !== null && (
        <div style={{ marginBottom: 10 }}>
          ...
          <div
            style={{ height: 5, borderRadius: 3, background: "var(--border)" }}
          >
            <div
              style={{
                height: "100%",
...
export default function BucketElevatorPage({ onResultsChange }) {
  ...
  const failCount =
    results?.checks?.filter((c) => c.type === "fail").length ?? 0;
  const warnCount =
    results?.checks?.filter((c) => c.type === "warn").length ?? 0;
  ...
  const middleSub = {
    ...
    checks: results
      ? `${results.checks?.length ?? 0} CEMA 375-2017 checks`
      : "—",
  }[activeTab];
  ...
  return (
    <>
          ...
          {[
            {
              ...
              color:
                results?.Q == null
                  ? "var(--muted)"
                  : Number(results.Q) >= Number(inputs.Q_req)
                    ? "var(--success)"
                    : "var(--danger)",
            },
            {
              label: "P",
              value: results?.P_total,
              dp: 1,
              unit: "kW",
              color:
                results?.P_total == null ? "var(--muted)" : "var(--warning)",
            },
            {
              label: "v",
              value: results?.v,
            ...
            const fmtd =
              k.value != null && !Number.isNaN(Number(k.value))
      ...
      {error && (
        <div
          className="warn-item w-fail"
          style={{ margin: "5px 10px", flexShrink: 0 }}
        >
          ⚠ API Error: {error}
        </div>
      )}
      ...
      >
        ...
        >
          ...
          {activeTab === "design" && (
            ...
            >
              {/* Schematic — takes remaining height above chart strip */}
              <div
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color:
                          chartTab === t.id ? "var(--primary)" : "var(--text3)",
        ...
        >
          ...
          >
            ...
            {activeTab === "checks" && <ChecksSummary results={results} />}
          </div>
        </div>
      </div>
