// ReportView.jsx — Executive + Detailed report view
// Pure display component — all values and status flags from results dict.
//
// v1.9.9: removed all frontend recomputation. Previously this file computed
// capOK, speedOK, crOK, T_total, failCount, warnCount, and motorMargin
// by scanning results.checks and doing arithmetic on raw fields. All of
// these now come from pre-computed backend fields:
//   results.cap_ok, results.speed_ok, results.cr_ok, results.l10_ok,
//   results.status, results.fail_count, results.warn_count,
//   results.T_total, results.motor_margin_pct
// The hardcoded "C basic = 355 kN" bearing constant has been removed from
// the detailed view — that value lives in the backend only.
...
function ExecutiveView({ results, inputs }) {
  ...
  // All flags pre-computed by backend — no recomputation here
  ...
  return (
    <div style={{ paddingBottom: 16 }}>
      ...
      <DataRow label="Shaft Material" value={results.shaft_material_name ?? "—"} mono={false} />
      <DataRow label="Shaft Section"  value={results.shaft_section ?? "—"} mono={false} />
...
function DetailedView({ results, inputs }) {
  ...
  return (
    <div style={{ paddingBottom: 16 }}>
      ...
      <DataRow label="Allowable shear"   value={results.shaft_tau_allow_MPa != null ? `${results.shaft_tau_allow_MPa} MPa` : "—"} />
      ...
      <DataRow label="Belt length"       value={results.belt_length_total_m != null ? `${results.belt_length_total_m} m` : "—"} />
      <DataRow label="Bucket count"      value={results.n_buckets != null ? `${results.n_buckets} off` : "—"} />
