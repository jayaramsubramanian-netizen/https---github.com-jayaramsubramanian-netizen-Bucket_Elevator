...
function Card({
  ...
  const badgeBg =
    badgeColor === "green"
      ? {
          bg: "rgba(31,184,110,.15)",
          col: T.success,
          bdr: "rgba(31,184,110,.3)",
        }
      : badgeColor === "orange"
        ? {
            bg: "rgba(217,142,0,.15)",
            col: T.warning,
            bdr: "rgba(217,142,0,.3)",
          }
        : badgeColor === "red"
          ? {
              bg: "rgba(224,82,82,.15)",
              col: T.danger,
              bdr: "rgba(224,82,82,.3)",
            }
          : badgeColor === "blue"
            ? {
                bg: "rgba(74,158,255,.12)",
                col: T.primary,
                bdr: "rgba(74,158,255,.25)",
              }
            : {
                bg: "rgba(90,122,154,.12)",
                col: T.text3,
                bdr: "rgba(90,122,154,.25)",
              };
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        ...
        {unit && (
          <span
            style={{ fontSize: 9, color: T.text3, flexShrink: 0, minWidth: 24 }}
          >
            {unit}
          </span>
        )}
      ...
      {note && (
        <div
          style={{ fontSize: 9, color: T.text3, marginTop: 3, lineHeight: 1.4 }}
        >
          {note}
        </div>
      )}
...
function TwoCol({ children }) {
  return (
    <div
      style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 10px" }}
    >
      {children}
    </div>
  );
...
function cardBadge(results, keywords) {
  ...
  if (matched.some((c) => c.type === "fail"))
    return { badge: "FAIL", color: "red" };
  if (matched.some((c) => c.type === "warn"))
    return { badge: "WARN", color: "orange" };
    ...
    >
      ...
      <div id="disc-bucket">
        <Card
          ...
          cema="CEMA 375  §6"
          badge={bkt.badge}
          badgeColor={bkt.color}
          defaultOpen={false}
        >
          <SubHead label="Selection Mode" />
          <Field
            label="Auto-select bucket"
            name="auto_bucket"
            type="toggle"
          ...
          {inp.auto_bucket && (
            <div style={{ fontSize: 9, color: T.text3, padding: "4px 0" }}>
              Bucket series selected automatically to meet Q_req at the
              specified belt speed.
            </div>
          )}
        </Card>
      </div>
      ...
      <div id="disc-advanced">
        ...
        >
          ...
          >
            CEMA 375 §4 Length Equivalency power method parameters. Set to 0 to
            use material-specific database defaults.
          </div>
          <Field
            label="Boot Pulley Dia."
