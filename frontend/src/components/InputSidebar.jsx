...
const DEFAULT_INPUTS = {
  ...
  Ceff: 0,
  bucket_gap: 25,
  wind_pressure_pa: 800,
  casing_t_override_mm: 0,
  shaft_material: "A36",
  shaft_d_override_mm: 0,
  belt_width_override_mm: 0,
  pulley_shell_t_override_mm: 0,
  bucket_thickness_override_mm: 0,
  takeup_screw_d_mm: 0,
  takeup_screw_len_m: 0,
  custom_rho: 0,
  custom_aor: 0,
  custom_abr: 0,
  custom_flowability: 0,
  custom_moisture: -1,
  custom_cohesion: -1,
  motor_kw_override: 0,
  chain_sf: 6.0,
  chain_sprocket_teeth: 0,
  boot_outlet_height_mm: 0,
};
          ...
          <input
            ...
            onChange={(e) => {
              ...
              onChange(
                ...
                type === "number"
                  ? _rv === "" || isNaN(_pf)
                    ? typeof DEFAULT_INPUTS[name] === "number"
                      ? DEFAULT_INPUTS[name]
                      : 0
                    : _pf
                  : _rv,
...
function checkBadge(results, keywords) {
  ...
  if (matched.some((c) => c.type === "fail"))
    return { label: "FAIL", color: T.danger };
  if (matched.some((c) => c.type === "warn"))
    return { label: "WARN", color: T.warning };
...
function SectionRow({ label, cema, summary, badge, onEdit, depth = 0 }) {
  return (
    <div
      ...
      onMouseEnter={(e) =>
        onEdit && (e.currentTarget.style.background = "rgba(255,255,255,.03)")
      }
...
function ProcessEdit({ inp, setField, results }) {
  ...
  return (
    <>
      ...
      {df && (
        ...
        >
              ...
              <div key={l}>
                ...
                <div
                  style={{
                    ...
                    color:
                      l === "Your value"
                        ? inp.fill_pct >= (r.min_fill_pct ?? 0) &&
                          inp.fill_pct <= (r.max_fill_pct ?? 100)
                          ? T.success
                          : T.danger
                        : l === "Recommended"
                          ? T.primary
          ...
          {r.min_fill_pct != null &&
            r.max_fill_pct != null &&
            (() => {
              const mn = r.min_fill_pct,
                mx = r.max_fill_pct;
              const rec = df.recommended_fill_pct,
                uv = inp.fill_pct;
              const pct = (v) =>
                Math.min(100, Math.max(0, ((v - 30) / 70) * 100));
          ...
          {df.spacing_status !== "optimal" && (
            <div style={{ fontSize: 10, color: T.warning, marginTop: 3 }}>
              ⚠ Spacing is {df.spacing_status.replace("_", " ")} — adjust Bucket
              Spacing Gap to approach {df.optimal_spacing_mm}mm
            </div>
          )}
        </div>
      )}
      ...
      >
        Leave at 0 / –1 to use database values. Override individual properties
        for site-specific conditions.
      </div>
      <F
        label="Custom Display Name"
        name="custom_mat_name"
        type="text"
        value={inp.custom_mat_name || ""}
        onChange={setField}
      ...
      <Row2>
        <F
          label="Abrasiveness 1–7"
          name="custom_abr"
          value={inp.custom_abr ?? 0}
          onChange={setField}
          min={0}
          max={7}
          step={1}
        ...
        />
        <F
          label="Flowability 1–4"
          name="custom_flowability"
          value={inp.custom_flowability ?? 0}
          onChange={setField}
          min={0}
          max={4}
          step={1}
          note="0=DB  1=Free  4=Sluggish"
        />
      </Row2>
      <Row2>
        <F
          label="Moisture %"
          name="custom_moisture"
          value={inp.custom_moisture ?? -1}
          onChange={setField}
          unit="%"
          min={-1}
          max={100}
    ...
    <>
      ...
      >
        ...
        <div style={{ marginTop: 8 }}>
          <F
            label="Snub pulley on return side (+30°)"
            name="snub_pulley"
            type="toggle"
            value={inp.snub_pulley ?? false}
            onChange={setField}
        ...
        {(inp.snub_pulley ?? false) && (
          <div style={{ fontSize: 10, color: T.success, marginTop: 4 }}>
            Snub active — effective wrap ≈{" "}
            {r.wrap_effective_deg != null ? `${r.wrap_effective_deg}°` : "210°"}
          </div>
        )}
      </div>
          ...
          >
            <div
              style={{
                ...
                fontWeight: 700,
                letterSpacing: ".05em",
                color: adequate ? T.success : T.warning,
                marginBottom: 4,
              }}
            >
            ...
            <div style={{ display: "flex", gap: 16, marginBottom: 4 }}>
              {[
                ...
                [
                  "Current",
                  r.wrap_effective_deg != null
                    ? `${r.wrap_effective_deg}°`
                    : `${inp.wrap_deg || 180}°`,
                ],
                ["Config", wr.config],
              ].map(([l, v]) => (
      ...
      <F
        label="Match head pulley diameter"
        name="boot_pulley_same_as_head"
        type="toggle"
        value={inp.boot_pulley_same_as_head ?? false}
        onChange={setField}
      />
      {!inp.boot_pulley_same_as_head &&
        (() => {
          ...
          return (
            <F
              label="Boot Pulley Diameter"
              name="boot_pulley_D_mm"
              value={inp.boot_pulley_D_mm}
              onChange={setField}
              unit="mm"
              min={100}
              max={1000}
              ...
              note={
                <span style={{ color: ratioOk ? undefined : T.warning }}>
                  Head:Boot ratio = {headBootRatio.toFixed(2)} (CEMA §3.2 limit
                  ≤ 2.00){!ratioOk ? " ⚠" : ""}
                </span>
              }
            />
        ...
        >
          {[
            ...
            [
              "Radial load",
              r.R_headshaft != null
                ? `${(r.R_headshaft / 1000).toFixed(1)} kN`
                : "—",
            ],
          ].map(([l, v]) => (
            <div key={l}>
      ...
      <F
        label="Shell Thickness Override"
        name="pulley_shell_t_override_mm"
        value={inp.pulley_shell_t_override_mm ?? 0}
        onChange={setField}
        unit="mm"
        min={0}
        max={50}
        step={1}
        note={
          results?.pulley_shell?.override_applied
...
function BeltEdit({ inp, setField, results }) {
  ...
  return (
    <>
      ...
      >
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          {[
            ...
            [
              "Eff. Tension",
              `${r.F_eff != null ? (r.F_eff / 1000).toFixed(1) : "—"} kN`,
            ],
          ].map(([l, v]) => (
            <div key={l}>
      ...
      <F
        label="Belt Width Override"
        name="belt_width_override_mm"
        value={inp.belt_width_override_mm ?? 0}
        onChange={setField}
        unit="mm"
        min={0}
        max={1500}
      ...
      />
      <F
        label="Belt Cover Grade"
        name="belt_grade"
        type="select"
        value={inp.belt_grade ?? ""}
        onChange={setField}
        options={[
...
const BUCKET_CATALOG = {
  AA: {
    ...
    sizes: [
      ...
      {
        id: "AA_18x10",
        label: "AA 18×10 — 17.84 L (W=457mm)",
        W: 457,
        P: 254,
        V: 17.84,
      },
  ...
  AC: {
    ...
    sizes: [
      {
        id: "AC_12x8",
        label: "AC 12×8×8  — 8.58 L  (W=305mm)",
        W: 305,
        P: 203,
        V: 8.58,
      },
      {
        id: "AC_14x8",
        label: "AC 14×8×8  — 10.08 L (W=356mm)",
        W: 356,
        P: 203,
        V: 10.08,
      },
      {
        id: "AC_16x8",
        label: "AC 16×8×8  — 11.55 L (W=406mm)",
        W: 406,
        P: 203,
        V: 11.55,
      },
      {
        id: "AC_18x10",
        label: "AC 18×10×10 — 19.57 L (W=457mm)",
        W: 457,
        P: 254,
        V: 19.57,
      },
      {
        id: "AC_20x10",
        label: "AC 20×10×10 — 21.75 L (W=508mm)",
        W: 508,
        P: 254,
        V: 21.75,
      },
      {
        id: "AC_24x10",
        label: "AC 24×10×10 — 26.08 L (W=610mm)",
        W: 610,
        P: 254,
        V: 26.08,
      },
  ...
  MF: {
    ...
    sizes: [
      {
        id: "MF_10x7",
        label: "MF 10×7×11  — 5.10 L  (W=254mm)",
        W: 254,
        P: 178,
        V: 5.1,
      },
      {
        id: "MF_12x7",
        label: "MF 12×7×11  — 6.17 L  (W=305mm)",
        W: 305,
        P: 178,
        V: 6.17,
      },
      {
        id: "MF_12x8",
        label: "MF 12×8×11  — 7.79 L  (W=305mm)",
        W: 305,
        P: 203,
        V: 7.79,
      },
      {
        id: "MF_14x8",
        label: "MF 14×8×11  — 9.20 L  (W=356mm)",
        W: 356,
        P: 203,
        V: 9.2,
      },
      {
        id: "MF_16x8",
        label: "MF 16×8×11  — 10.62 L (W=406mm)",
        W: 406,
        P: 203,
        V: 10.62,
      },
      {
        id: "MF_18x8",
        label: "MF 18×8×11  — 11.89 L (W=457mm)",
        W: 457,
        P: 203,
        V: 11.89,
      },
      {
        id: "MF_24x10",
        label: "MF 24×10×11 — 24.07 L (W=610mm)",
        W: 610,
        P: 254,
        V: 24.07,
      },
  ...
  HF: {
    ...
    sizes: [
      {
        id: "HF_10x7",
        label: "HF 10×7×11 — 5.38 L  (W=254mm)",
        W: 254,
        P: 178,
        V: 5.38,
      },
      {
        id: "HF_12x7",
        label: "HF 12×7×11 — 6.80 L  (W=305mm)",
        W: 305,
        P: 178,
        V: 6.8,
      },
      {
        id: "HF_14x7",
        label: "HF 14×7×11 — 7.93 L  (W=356mm)",
        W: 356,
        P: 178,
        V: 7.93,
      },
      {
        id: "HF_14x8",
        label: "HF 14×8×11 — 9.91 L  (W=356mm)",
        W: 356,
        P: 203,
        V: 9.91,
      },
      {
        id: "HF_16x8",
        label: "HF 16×8×11 — 11.19 L (W=406mm)",
        W: 406,
        P: 203,
        V: 11.19,
      },
      {
        id: "HF_18x8",
        label: "HF 18×8×11 — 12.83 L (W=457mm)",
        W: 457,
        P: 203,
        V: 12.83,
      },
  ...
  SC: {
    ...
    sizes: [
      {
        id: "SC_12x8",
        label: "SC 12×8×11 — 15.3 L  (W=305mm)",
        W: 305,
        P: 222,
        V: 15.29,
      },
      {
        id: "SC_14x8",
        label: "SC 14×8×11 — 17.8 L  (W=356mm)",
        W: 356,
        P: 222,
        V: 17.84,
      },
      {
        id: "SC_16x8",
        label: "SC 16×8×11 — 20.4 L  (W=406mm)",
        W: 406,
        P: 222,
        V: 20.39,
      },
      {
        id: "SC_18x8",
        label: "SC 18×8×11 — 22.9 L  (W=457mm)",
        W: 457,
        P: 222,
        V: 22.94,
      },
      {
        id: "SC_20x8",
        label: "SC 20×8×11 — 25.5 L  (W=508mm)",
        W: 508,
        P: 222,
        V: 25.49,
      },
      {
        id: "SC_20x12",
        label: "SC 20×12×17 — 54.9 L (W=508mm)",
        W: 508,
        P: 324,
        V: 54.93,
      },
      {
        id: "SC_24x12",
        label: "SC 24×12×17 — 66.0 L (W=610mm)",
        W: 610,
        P: 324,
        V: 65.98,
      },
      {
        id: "SC_30x12",
        label: "SC 30×12×17 — 82.4 L (W=762mm)",
        W: 762,
        P: 324,
        V: 82.4,
      },
...
function BucketEdit({ inp, setField, results }) {
  ...
  const sizesToShow =
    filteredSizes.length > 0 ? filteredSizes : styleData.sizes; // fallback

  // When style changes, auto-select the middle size within filtered list
  const handleStyleChange = (newStyle) => {
    setSelectedStyle(newStyle);
    const sizes =
      BUCKET_CATALOG[newStyle]?.sizes.filter((s) => s.W <= maxBucketW) || [];
  ...
  const currentSize =
    sizesToShow.find((s) => s.id === inp.bucket_id) ||
    sizesToShow[Math.floor(sizesToShow.length / 2)];

  const inputStyle = {
    background: "var(--panel2,#132238)",
  ...
  return (
    <>
      ...
      <F
        label="Auto-select smallest adequate series"
        name="auto_bucket"
        type="toggle"
        value={inp.auto_bucket}
        onChange={setField}
      ...
      {results?.bucket_recommendation &&
        (() => {
          ...
          const isCurrent =
            styleFromId(inp.bucket_id) === rec.recommended_style;
          return (
            <div
              style={{
            ...
            >
              <div
                style={{
                  ...
                  fontWeight: 700,
                  letterSpacing: ".05em",
                  color: isCurrent ? T.success : T.primary,
                  marginBottom: 4,
                }}
              >
                {isCurrent
                  ? "✓ CURRENT STYLE MATCHES RECOMMENDATION"
              ...
              <div
                style={{
                  ...
                  color: T.text3,
                  lineHeight: 1.5,
                  marginBottom: rec.notes?.length ? 6 : 0,
                }}
              >
                {rec.reasoning}
              </div>
              {rec.notes?.map((n, i) => (
                <div
                  key={i}
                  style={{ fontSize: 10, color: T.warning, marginTop: 3 }}
                >
                  ⚠ {n}
                </div>
              ))}
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <select
          ...
          onChange={(e) => {
            setField("bucket_id", e.target.value);
            setField("auto_bucket", false);
          }}
        ...
        {hiddenCount > 0 && (
          ...
          >
            ⚠ {hiddenCount} size{hiddenCount > 1 ? "s" : ""} hidden — wider than
            belt ({resolvedBeltW}mm − 50mm clearance). Increase belt width
            override to unlock them.
          </div>
        )}
        {inp.auto_bucket && (
          <div
        ...
        >
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            {[
              ...
              <div key={l}>
                <div style={{ fontSize: 10, color: "var(--text3,#5a7a9a)" }}>
                  {l}
                </div>
                <div
                  style={{
      ...
      <F
        label="Bucket Spacing Gap"
        name="bucket_gap"
        value={inp.bucket_gap}
        onChange={setField}
        unit="mm"
        min={0}
        max={200}
        ...
        >
          <div style={{ display: "flex", gap: 16 }}>
            {[
              [
                "Catalogue gauge",
                `${results.bucket_thickness.t_implied_mm} mm`,
              ],
      ...
      <F
        label="Plate Thickness Override"
        name="bucket_thickness_override_mm"
        value={inp.bucket_thickness_override_mm ?? 0}
        onChange={setField}
        unit="mm"
        min={0}
        max={20}
...
function TakeupEdit({ inp, setField, results }) {
  ...
  return (
    <>
        ...
        {["gravity", "screw", "auto"].map((v) => (
          <button
            ...
            style={{
              ...
              background:
                inp.takeup_type === v ? "rgba(74,158,255,.15)" : T.panel2,
        ...
        >
          ...
          <div style={{ display: "flex", gap: 16 }}>
            {[
              [
                "Counterweight",
                `${tg.W_counterweight_kg_gross?.toFixed(0) ?? "—"} kg`,
              ],
              [
                "Travel",
                `${tg.travel_m != null ? (tg.travel_m * 1000).toFixed(0) : "—"} mm`,
              ],
            ].map(([l, v]) => (
              <div key={l}>
                <div style={{ fontSize: 10, color: T.text3 }}>{l}</div>
      ...
      <Row2>
        <F
          label="Screw Core Dia."
          name="takeup_screw_d_mm"
          value={inp.takeup_screw_d_mm ?? 0}
          onChange={setField}
          unit="mm"
          min={0}
          max={200}
        ...
        <F
          label="Screw Span"
          name="takeup_screw_len_m"
          value={inp.takeup_screw_len_m ?? 0}
          onChange={setField}
          unit="m"
          min={0}
          max={5}
      ...
      {ts.d_core_recommend_mm > 0 && (
        ...
        >
          ✓ Recommended: {ts.d_core_recommend_mm} mm core (SF_buckling ={" "}
          {ts.SF_buckling?.toFixed(2) ?? "—"})
        </div>
      )}
    </>
  );
...
function ShaftEdit({ inp, setField, results }) {
  return (
    <>
      ...
      <F
        label="Shaft Material Grade"
        name="shaft_material"
        type="select"
        value={inp.shaft_material ?? "A36"}
        onChange={setField}
        options={[
          { value: "A36", label: "A36 Mild Steel (τ=42 MPa) — standard" },
          {
            value: "1045_HR",
            label: "1045 Hot-Rolled (τ=52 MPa) — higher capacity",
          },
          {
            value: "1045_CD",
            label: "1045 Cold-Drawn (τ=70 MPa) — precision machined",
          },
          {
            value: "4140_QT",
            label: "4140 Q&T Alloy (τ=110 MPa) — heavy/impact duty",
          },
        ]}
        note={
          results?.shaft_material_name
      ...
      <F
        label="Head Shaft Dia. Override"
        name="shaft_d_override_mm"
        value={inp.shaft_d_override_mm ?? 0}
        onChange={setField}
        unit="mm"
        min={0}
        max={500}
...
function DischargeEdit({ inp, setField, results }) {
  ...
  return (
    <>
      ...
      >
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          {[
            ...
            [
              "θ release",
              r.theta_rel != null ? `${Number(r.theta_rel).toFixed(1)}°` : "—",
            ],
      ...
      >
        Discharge geometry is calculated from pulley diameter and belt speed. To
        adjust: change D_mm or n_rpm in Head &amp; Tail Pulley. To switch to
        continuous discharge (HF), select HF bucket series in Bucket Selection.
      </div>
      ...
      >
        ...
        Lower-friction liners reduce the minimum chute angle required for mass
        flow, resolving funnel-flow and plugging warnings without geometry
        changes.
      </div>
      {/* Liner options */}
      {[
        {
          ...
          mu: "—",
          phi: "—",
          note: "CEMA wear-index selection",
        },
        {
          id: "mild_steel",
          label: "Mild Steel (unlined)",
          mu: "0.55",
          phi: "28.8°",
          note: "Class 1–2, v < 1.5 m/s",
        },
        {
          id: "ar400",
          label: "AR400 Wear Plate",
          mu: "0.48",
          phi: "25.7°",
          note: "Abrasive Class 3+",
        },
        {
          id: "nat_rubber",
        ...
        },
        {
          ...
          mu: "0.20",
          phi: "11.3°",
          note: "Sticky/cohesive — lowest min angle",
        },
        {
          id: "ceramic_tile",
          label: "Ceramic Tile",
      ...
      {results?.discharge_chute?.maintenance && (
        ...
        >
          ...
          <span style={{ color: T.text3, marginLeft: 12 }}>
            Min chute angle:{" "}
          </span>
...
function CasingEdit({ inp, setField, results }) {
  ...
  return (
    <>
      ...
      <F
        label="Plate Thickness Override"
        name="casing_t_override_mm"
        value={inp.casing_t_override_mm ?? 0}
        onChange={setField}
        unit="mm"
        min={0}
        max={50}
...
function ServiceEdit({ inp, setField }) {
  return (
    <>
      ...
      />
      <F
        label="Pulley Lagging Friction μ"
        name="mu"
        value={inp.mu}
        onChange={setField}
        min={0.1}
...
function PowerEdit({ inp, setField, results }) {
  ...
  const reqRatio =
    inp.n_rpm > 0 ? Math.round(1450 / Math.max(inp.n_rpm, 1)) : 0;
  ...
  return (
    <>
      ...
      />
      <F
        label="Motor kW Override"
        name="motor_kw_override"
        value={inp.motor_kw_override ?? 0}
        onChange={setField}
        unit="kW"
        min={0}
        max={1000}
      ...
      <ComponentPicker
        ...
        autoLabel={
          torqueNm > 0
            ? `Tn ≥ ${torqueNm.toFixed(0)} Nm, ratio ≈ ${reqRatio}:1`
            : null
        }
      ...
      >
        Leq and Ceff default to material database values. Set to 0 to use
        defaults. Boot pulley diameter is set in Head &amp; Tail Pulley.
      </div>
      <Row2>
        <F
          label="Leq Factor"
...
export default function InputSidebar({ inputs, setField, results }) {
  ...
  const bootDia = inp.boot_pulley_same_as_head
    ? inp.D_mm
    : (inp.boot_pulley_D_mm ?? "—");
  const bkt = r.bucket || {};
  ...
  const summaries = {
    ...
    shaft: `${inp.shaft_material ?? "A36"} · Ø${r.d_mm ?? "—"}mm · ${r.governed_by ?? "—"}`,
  ...
  const modals = {
    process: {
      ...
      cema: "CEMA 375 §4",
      C: <ProcessEdit inp={inp} setField={setField} results={r} />,
    },
    pulleys: {
      title: "Head & Tail Pulley",
      cema: "CEMA 375 §3,6",
    ...
    bucket: {
      ...
      cema: "CEMA 375 §6",
      C: <BucketEdit inp={inp} setField={setField} results={r} />,
    },
    takeup: {
      title: "Take-Up Selection",
      cema: "CEMA 375 §4",
      C: <TakeupEdit inp={inp} setField={setField} results={r} />,
    },
    shaft: {
      title: "Shaft Design",
      cema: "CEMA 375 §4",
      C: <ShaftEdit inp={inp} setField={setField} results={r} />,
    },
    discharge: {
      title: "Discharge Section",
      cema: "CEMA 375 §5",
    ...
    casing: {
      title: "Casing Design",
      cema: "CEMA 375 §7",
      C: <CasingEdit inp={inp} setField={setField} results={r} />,
    },
    service: {
      title: "Service Conditions",
      cema: "v1.3.0",
      C: <ServiceEdit inp={inp} setField={setField} />,
    },
    power: {
      title: "Power Transmission",
      cema: "CEMA 375 §4",
      C: <PowerEdit inp={inp} setField={setField} results={r} />,
    },
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <SectionRow
          label="Take-Up Selection"
          depth={1}
          badge={badges.takeup}
          summary={summaries.takeup}
          onEdit={() => open("takeup")}
        />
        <SectionRow
          label="Shaft Design"
          depth={1}
          badge={badges.shaft}
          summary={summaries.shaft}
          onEdit={() => open("shaft")}
        />
        <SectionRow
          label="Discharge Section"
          depth={1}
          badge={badges.discharge}
          summary={summaries.discharge}
        ...
        <SectionRow
          ...
          badge={
            r.feed_design
              ? r.feed_design.warnings?.length
                ? { label: "WARN", color: T.warning }
                : { label: "OK", color: T.success }
