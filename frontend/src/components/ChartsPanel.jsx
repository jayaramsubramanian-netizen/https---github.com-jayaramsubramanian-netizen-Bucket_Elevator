...
const C = {
  ...
  amber: "#f59e0b",   teal: "#14b8a6",      text: "#f1f5f9",      red: "#ef4444",
};

const TS = { background: C.panel, border: `1px solid ${C.border2}`, borderRadius: "var(--r-md)", fontFamily: "JetBrains Mono", fontSize: 11 };

export default function ChartsPanel({ results, inputs, activeTab }) {
  // activeTab prop: when provided by parent (3-col layout), use it.
  ...
  return (
    <div>
      {/* Only render own tab bar when not controlled by parent */}
      {!activeTab && (
          ...
          <>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={results.speed_sweep} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Line yAxisId="right" type="monotone" dataKey="power" name="Power (kW)" stroke={C.series2} strokeWidth={2} dot={false} />
          ...
          <>
            ...
            <div style={{ padding: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {[
                ...
                { label: "Tight Side (T₁)",  value: (results.T1 / 1000).toFixed(3) + " kN", color: C.series2 },
                ...
                <div key={i} style={{ background: C.hi, borderRadius: "var(--r-md)", padding: "10px 12px", border: `1px solid ${C.border2}` }}>
