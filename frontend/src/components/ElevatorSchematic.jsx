  ...
  return (
    <g>
      ...
      <text x={bx + 4} y={by + 25} fontSize={7} fill={C.text3}>
        BUCKET: {bkt.id ?? "—"}×{bkt.W ?? "—"} {bkt.W ?? "—"}×{bkt.H ?? "—"}mm ·
        H = {inp.H_m ?? "—"} m
      </text>
      <text x={bx + 4} y={by + 41} fontSize={7} fill={C.text3}>
        {view.toUpperCase()} VIEW
      </text>
      <text x={bx + 104} y={by + 41} fontSize={7} fill={C.text3}>
        D = {inp.D_mm ?? "—"} mm
      </text>
      <text x={bx + 4} y={by + 52} fontSize={6} fill={C.label}>
    ...
    return (
      <g>
        ...
        <text
          ...
          fill={C.labelBr}
          textAnchor="middle"
          transform={`rotate(-90,${dx - sgn * 3},${(y1 + y2) / 2})`}
        >
          {label}
        </text>
      </g>
...
function ElevationView({ inputs, results, W, H, hovered, setHovered }) {
  ...
  const headD = Number(inp.D_mm || 500),
    bootD = Number(inp.boot_pulley_D_mm || 300);
  ...
  const casW = rH + 18;
  const topY = margin.top + rH,
    botY = H - margin.bottom - rB,
    elevH = botY - topY;
  const bltL = cx - rH * 0.6,
    bltR = cx + rH * 0.6;
  const spacePx = r.spacing
    ? r.spacing * (elevH / Math.max(inp.H_m || 25, 1))
    : 52;
  ...
  const carryBuckets = Array.from(
    { length: nBkts },
    (_, i) => botY - 10 - i * spacePx,
  );
  const returnBuckets = Array.from(
    { length: Math.ceil(nBkts / 2) },
    (_, i) => topY + 20 + i * spacePx * 1.3,
  );
  ...
  const callouts = {
    ...
    bucket: {
      title: `BUCKET ${bkt.id ?? "—"}`,
      lines: [
        `${bkt.W ?? "—"}×${bkt.H ?? "—"}mm P=${bkt.P ?? "—"}mm`,
        `V = ${bkt.V ?? "—"} L`,
        `Spacing = ${r.spacing ? Math.round(r.spacing * 1000) : "—"} mm`,
        `Fill: ${inp.fill_pct ?? "—"}%`,
      ],
    },
  };
  ...
  return (
    ...
    >
      ...
      <text x={cx + rH + 64} y={topY - 12} fontSize={7} fill={C.label}>
        CHUTE
      </text>
      ...
      {spacePx > 14 && (
        <Dim
          ...
          x2={cx + casW + 8}
          y2={botY - 10 - spacePx}
          label={`${r.spacing ? Math.round(r.spacing * 1000) : "—"}mm`}
          offset={20}
          side="positive"
        />
      )}
      ...
      <text x={cx} y={21} fontSize={7} fill={C.label} textAnchor="middle">
        Ø{headD}mm · {inp.n_rpm ?? "—"}rpm · Lagged
      </text>
      ...
      <text x={cx + rB + 6} y={botY + 4} fontSize={7.5} fill={C.text3}>
        DB = {headD} mm
      </text>
      ...
      <text
        ...
        textAnchor="middle"
      >
        BUCKET {bkt.id ?? "—"} · {bkt.W ?? "—"}×{bkt.H ?? "—"}mm ·{" "}
        {bkt.V ?? "—"}L · SPACING{" "}
        {r.spacing ? Math.round(r.spacing * 1000) : "—"}mm
      </text>
...
function PlanView({ inputs, results, W, H, hovered, setHovered }) {
  ...
  return (
    ...
    >
      ...
      <g
        onMouseEnter={() => setHovered("casing")}
        onMouseLeave={() => setHovered(null)}
        style={{ cursor: "help" }}
      >
        {/* Full casing outline */}
      ...
      >
        ...
        <rect
          x={cx - pl / 2 - 5 * scale}
          y={cy - (D * scale) / 2 - 6 * scale}
          width={pl + 10 * scale}
          height={D * scale + 12 * scale}
          fill="none"
          stroke={C.lagging}
          strokeWidth={3.5}
          opacity={0.6}
      ...
      {[-2, -1, 0, 1, 2].map((i) => {
        ...
        const bktSpacing = Number(r.spacing ?? 0.2) * 1000 * scale * 0.4;
        return (
          <rect
            key={i}
            x={cx - bktW_plan / 2}
            y={cy - bktD_plan / 2 + i * bktSpacing}
            width={bktW_plan}
            height={bktD_plan}
            fill={C.bucket}
            fillOpacity={0.6}
      ...
      <Dim
        ...
        x2={cx + pl / 2}
        y2={cy - cd / 2 - 42}
        label={`Pulley face = ${PL.toFixed(0)} mm`}
        offset={16}
      />
      ...
      {hovered === "head" && (
        <Callout
          ...
          lines={[
            `D = ${D.toFixed(0)} mm`,
            `Pulley face = ${PL.toFixed(0)} mm`,
            `Lagged — amber ring`,
            `Shaft Ø${f(dShaft, 0)} mm`,
          ]}
        />
      )}
      {hovered === "bearings" && (
      ...
      {hovered === "casing" && (
        <Callout
          ...
          H={H}
          lines={[
            `Width: ${CW.toFixed(0)} mm  (BW + 120)`,
            "Depth: 400 mm (typical)",
            `H = ${inp.H_m ?? "—"} m total`,
          ]}
        />
      )}
    ...
    >
      ...
      {[-1, 1].map((side) => (
        <g key={side}>
          {/* Vertical leg */}
          <rect
            x={cx + side * (casDepth / 2 + 4)}
            y={topY + elevH_px * 0.15}
            width={10 * scale}
            height={elevH_px * 0.85}
            fill={C.leg}
            stroke={C.dim}
            strokeWidth={0.8}
          />
          {/* Diagonal brace */}
          <line
            x1={cx + side * (casDepth / 2 + 4 + (side > 0 ? 10 * scale : 0))}
            y1={topY + elevH_px * 0.15}
            x2={cx + side * (casDepth / 2 + 8 + 30 * scale)}
            y2={topY + elevH_px * 0.6}
            stroke={C.leg}
            strokeWidth={1.5}
            opacity={0.7}
          />
      ...
      {[0.25, 0.55, 0.78].map((frac, i) => (
        <rect
          key={i}
          x={cx - casDepth / 4}
          y={topY + elevH_px * frac}
          width={casDepth / 2}
          height={elevH_px * 0.08}
          fill="none"
          stroke={C.dimTxt}
          strokeWidth={0.8}
          strokeDasharray="3 2"
      ...
      <Dim
        ...
        x2={cx + casDepth / 2}
        y2={topY - rH_px / 3 - 24}
        label={`Casing depth = ${CD.toFixed(0)} mm`}
        offset={18}
      />
...
function TrajectoryView({ inputs, results, W, H }) {
  const r = results || {};
  const traj = r.trajectory || [],
    upper = r.trajectory_upper || [],
    lower = r.trajectory_lower || [];
  ...
  const yMin = Math.min(...allY),
    yMax = Math.max(...allY);
  const xR = Math.max(xMax - xMin, 0.01),
    yR = Math.max(yMax - yMin, 0.01);
  ...
  return (
    ...
    >
        ...
        <>
          ...
          <path
            d={
              toPath(upper) +
              " " +
              toPath([...lower].reverse()).replace("M", "L")
            }
            fill={C.danger}
            fillOpacity={0.06}
          />
      ...
      {traj[0] && (
        <circle cx={tx(traj[0].x)} cy={ty(traj[0].y)} r={4} fill={C.danger} />
      )}
      <text
        x={W / 2}
        y={22}
      ...
      <text x={W / 2} y={36} fontSize={8} fill={C.label} textAnchor="middle">
        Throw {f(m.throw_distance_m, 3)} m · CR = {f(results?.cr, 3)} · θ ={" "}
        {f(results?.theta_rel, 1)}° from vertical
      </text>
      <text
        x={W / 2}
        y={H - 8}
...
function BucketDetailView({ inputs, results, W, H }) {
  ...
  // Bolt positions
  const boltSW = cat.boltSpacW * scale,
    boltSH = cat.boltSpacH * scale;
  const bolts = [
  ...
  const n_buckets = r.spacing
    ? Math.ceil((2 * Number(inputs?.H_m || 25)) / r.spacing)
    : 0;
  ...
  return (
    ...
    >
      ...
      <path
        d={`M ${fX} ${fY} Q ${fX - backR * 0.35} ${midY} ${fX} ${fY + bHs}`}
        fill="none"
        stroke={C.lagging}
        strokeWidth={2}
        opacity={0.7}
      ...
      <path
        d={`M ${sX} ${sY} Q ${sX - 10} ${sY + bHs * 0.5} ${sX} ${sY + bHs}`}
        fill="none"
        stroke={C.lagging}
        strokeWidth={2}
        opacity={0.7}
      ...
      {[
        ...
        <g key={i}>
          ...
          />
          <text x={18} y={H - 120 + i * 14} fontSize={8} fill={C.text3}>
            {k}
          </text>
          <text
            x={130}
...
export default function ElevatorSchematic({ inputs, results }) {
  ...
  useEffect(() => {
    ...
    const h = (e) => {
      e.preventDefault();
      setZoom((z) =>
        Math.min(4, Math.max(0.25, z * (e.deltaY < 0 ? 1.12 : 0.89))),
      );
    };
    ...
    >
      ...
      >
        {VIEWS.map((v) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              background:
                view === v.id ? "rgba(59,130,246,.12)" : "transparent",
        ...
        {["+", "−"].map((l, i) => (
          <button
            ...
            onClick={() =>
              setZoom((z) =>
                Math.min(4, Math.max(0.25, z * (i === 0 ? 1.2 : 0.8))),
              )
            }
            style={{
              width: 22,
      ...
      >
        {[
          ...
          >
            <div
              style={{ fontSize: 7.5, color: C.text3, letterSpacing: ".06em" }}
            >
              {k.label}
            </div>
            <div
      ...
      >
        ...
        <span style={{ color: hovered ? C.primary : C.text3 }}>
          {hovered
            ? `${hovered.toUpperCase()} — hover for callout`
            : "Hover components for engineering details"}
        </span>
      </div>
    </div>
  );
